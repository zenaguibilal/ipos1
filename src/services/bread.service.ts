...
class BreadService {
    ...
    async generateAndGetOrdersForDate(
        ...
        // FIX #16 : un seul ordre manuel (customerUuid != null) ne doit pas
        // bloquer la génération automatique pour TOUS les autres clients.
        // On compte les clients actifs et les ordres auto déjà générés
        // (non-manuels) séparément.
        const activeBreadClientsCount = await db.customers
            .where('isBreadClient')
            .equals(1)
            .count();

        // Les ordres auto ne portent pas le flag isManual = true
        ...
        for (const client of activeBreadClients) {
            ...
            if (quantity > 0) {
                ordersToCreate.push({
                    ...
                    date,
                    quantite:   quantity,
                    isManual:   false,   // marqueur pour distinguer auto vs manuel
                    est_paye:   false,
                    est_livre:  false,
                    venteUuid:  null,
                    createdAt:  new Date(),
    ...
    async addManualBreadOrder(params: {
        ...
        const newOrder: BreadOrder = {
            ...
            date,
            quantite:     quantity,
            isManual:     true,   // FIX #16 : marqueur ordre manuel
            est_paye:     false,
            est_livre:    false,
            venteUuid:    null,
            createdAt:    new Date(),
    ...
    async convertBreadOrdersToSales(
        ...
        // FIX #13 : déclarer TOUTES les tables accédées par salesService.createSale
        // et customerService.recalculateCustomerStatus à l'intérieur de la transaction.
