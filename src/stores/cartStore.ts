...
interface CartActions {
    ...
    /**
     * Updates item quantity with floating point support and
     * high-precision stock validation.
     */
        ...
        (set, get) => ({
            ...
            actions: {
                ...
                addItemToCart: (product, quantity = 1) => {
                    ...
                    // Clear flash animation after 500 ms
                    setTimeout(() => {
                        set(
                            produce((state: CartState) => {
                                const targetCart = state.carts.find(
                        ...
                        produce((state: CartState) => {
                            ...
                            if (item) {
                                ...
                                if (
                                    isStockedItem &&
                                    newQuantity >
                                        item.quantity + FINANCIAL_EPSILON
                                ) {
                                    toast.error('Stock insuffisant', {
                                        description: `Max: ${item.quantity}`,
                                    });
