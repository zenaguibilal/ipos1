...
const StatCard = React.memo(({ title, value, icon: Icon, change, isLoading, href, positiveIsGood = true, suffix }: { title: string, value: string, icon: React.ElementType, change?: number, isLoading: boolean, href?: string, positiveIsGood?: boolean, suffix?: string }) => {
    ...
    const cardContent = (
        <Card className="app-card h-full bg-card/40 backdrop-blur-sm overflow-hidden group border-white/5 rounded-lg">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-3 p-6">
                ...
                <div className="p-3 rounded-2xl bg-muted/50 text-muted-foreground group-hover:bg-primary group-hover:text-primary-foreground group-hover:shadow-lg group-hover:shadow-sm transition-all duration-500">
            ...
            <CardContent className="px-6 pb-6">
                {isLoading ? (
                    ...
                    <div className="flex items-baseline gap-1">
                        <div className="text-xl font-semibold tracking-tighter text-foreground group-hover:scale-105 transition-transform duration-500 origin-left">{value}</div>
                        {suffix && <span className="text-[10px] font-semibold text-muted-foreground uppercase opacity-50">{suffix}</span>}
                    </div>
                )}
                {isLoading ? (
                    ...
                    (change !== undefined && isFinite(change) && change !== 0) ? (
                        <div className="mt-3 flex items-center gap-2">
                            <div className={cn(
                                "flex items-center gap-0.5 px-2 py-1 rounded-xl text-[10px] font-semibold tracking-tighter shadow-sm",
                            ...
                            <span className="text-[9px] font-semibold text-muted-foreground uppercase opacity-30 tracking-wide">Évolution</span>
                        </div>
...
const SalesChart = React.memo(({ data, isLoading }: { data: SalesByDay[], isLoading: boolean }) => (
    <Card className="app-card lg:col-span-2 bg-card/40 backdrop-blur-sm border-white/5 overflow-hidden rounded-lg">
        <CardHeader className="bg-muted/20 border-b border-white/5 p-4">
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <CardTitle className="text-lg font-semibold tracking-tighter">Courbe de Croissance</CardTitle>
                        <CardDescription className="text-[10px] font-semibold uppercase text-primary/50">Flux de revenus & Bénéfice net</CardDescription>
                    </div>
                </div>
            </div>
        </CardHeader>
        <CardContent className="h-96 w-full p-4">
             {isLoading ? (
                <div className="h-full w-full flex flex-col items-center justify-center bg-muted/5 rounded-lg border border-dashed border-white/5">
                    ...
                    <p className="text-[10px] font-semibold uppercase text-primary/20">Analyse des flux...</p>
                </div>
            ) : (
            <ResponsiveContainer>
...
const RecentActivity = React.memo(({ sales, returns, isLoading }: { sales: RecentSale[], returns: RecentReturn[], isLoading: boolean }) => (
    <Card className="app-card bg-card/40 backdrop-blur-sm border-white/5 overflow-hidden rounded-lg">
        <CardHeader className="bg-muted/20 border-b border-white/5 p-4">
            <CardTitle className="text-lg font-semibold tracking-tighter">Flux en Temps Réel</CardTitle>
            <CardDescription className="text-[10px] font-semibold uppercase text-muted-foreground/40">Derniers mouvements validés</CardDescription>
        </CardHeader>
                ...
                <div className="space-y-3">
                    {sales.length === 0 && returns.length === 0 ? (
                        <div className="py-20 text-center flex flex-col items-center gap-4 opacity-20">
                            ...
                            <p className="text-[10px] font-semibold uppercase ">En attente de flux</p>
                        </div>
                    ) : (
                        <>
                            {sales.map(s => (
                                <Link href={`/sales-history?query=${s.invoiceNumber}`} key={`sale-act-${s.uuid}`} className="flex items-center gap-4 p-4 rounded-lg bg-muted/20 border border-transparent hover:border-primary/30 hover:bg-primary/5 transition-all duration-500 group">
                                    <div className="p-3 rounded-2xl bg-primary/10 text-primary group-hover:scale-110 transition-all shadow-inner">
                                        <Receipt className="h-5 w-5" />
                                    </div>
                                    <div className="flex-grow min-w-0">
                                        <p className="font-semibold text-sm tracking-tight truncate group-hover:text-primary transition-colors">{s.customerName}</p>
                                        <p className="text-[9px] font-semibold text-muted-foreground/40 uppercase tracking-wide">{format(safeToDate(s.createdAt!), 'HH:mm')} • #{s.invoiceNumber}</p>
                                    </div>
                                    <div className="text-right">
                                        <p className="font-semibold text-base text-primary tracking-tighter">{formatCurrency(s.total)}</p>
                                    </div>
                                </Link>
                            ))}
                            {returns.map(r => (
                                <Link href={`/returns?query=${r.originalInvoiceNumber}`} key={`return-act-${r.uuid}`} className="flex items-center gap-4 p-4 rounded-lg bg-destructive/5 border border-transparent hover:border-destructive/30 hover:bg-destructive/10 transition-all duration-500 group">
                                    <div className="p-3 rounded-2xl bg-destructive/10 text-destructive group-hover:scale-110 transition-all shadow-inner">
                                        <Undo2 className="h-5 w-5" />
                                    </div>
                                    <div className="flex-grow min-w-0">
                                        <p className="font-semibold text-sm tracking-tight truncate">{r.customerName}</p>
                                        <p className="text-[9px] font-semibold text-muted-foreground/40 uppercase tracking-wide">{format(safeToDate(r.createdAt!), 'HH:mm')} • Retour Client</p>
                                    </div>
                                    <div className="text-right">
                                        <p className="font-semibold text-base text-destructive tracking-tighter">-{formatCurrency(r.totalReturnValue)}</p>
                                    </div>
                                </Link>
                            ))}
                        </>
...
export default function DashboardPage() {
    ...
    return (
        <div className="p-6 sm:p-4 space-y-4 max-w-[1800px] mx-auto animate-in fade-in duration-1000">
            ...
            <div className="grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-4 2xl:grid-cols-8">
                <StatCard title="Chiffre d'Affaires" value={formatCurrency(data?.stats.totalRevenue ?? 0)} icon={TrendingUp} isLoading={isLoading} change={data?.stats.totalRevenueChange} href="/sales-history" />
                ...
                <StatCard title="Charges Totales" value={formatCurrency(data?.stats.totalExpenses ?? 0)} icon={Wallet} isLoading={isLoading} change={data?.stats.totalExpensesChange} positiveIsGood={false} href="/expenses" />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                <Card className="app-card bg-card/40 backdrop-blur-sm border-white/5 overflow-hidden rounded-lg">
                    <CardHeader className="bg-muted/20 border-b border-white/5 p-4">
                        <CardTitle className="text-xl font-semibold tracking-tighter flex items-center gap-3">
                        ...
                        {isLoading ? [...Array(5)].map((_, i) => <Skeleton key={`skel-prod-${i}`} className="h-9 w-full rounded-2xl bg-muted/10" />) : 
                            data?.topProducts.map((p, i) => (
                                <div key={`prod-list-${p.productUuid}`} className="flex items-center gap-5 group">
                                    <span className="text-lg font-semibold text-muted-foreground/20 w-8">0{i + 1}</span>
                                    <div className="flex-grow">
                                        <p className="font-semibold text-sm group-hover:text-primary transition-colors">{p.name}</p>
                                        <span className="text-[9px] font-semibold uppercase text-muted-foreground/40">{p.quantitySold} unités vendues</span>
                                    </div>
                                    <p className="font-semibold text-sm tracking-tighter">{formatCurrency(p.revenueGenerated)}</p>
                                </div>
                            ))
                        }
                    </CardContent>
                            ...
                            data?.topCustomers.map((c, i) => (
                                <Link href={`/customers/${c.customerUuid}`} key={`cust-list-${c.customerUuid}`} className="flex items-center gap-5 group">
                                    ...
                                    <div className="flex-grow">
                                        ...
                                        <p className="text-[9px] font-semibold uppercase text-muted-foreground/40 mt-1">Rang #{i+1}</p>
                                    </div>
                                    <p className="font-semibold text-sm text-primary">{formatCurrency(c.totalSpent)}</p>
                                </Link>
                            ))
                        }
                    </CardContent>
