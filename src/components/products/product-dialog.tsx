...
import { Loader2, X, AlertTriangle, ChevronsUpDown, Plus, Package, Tag, Box } from 'lucide-react';
                ...
                <form onSubmit={handleSubmit}>
                    <DialogHeader className="bg-primary/5 p-6 border-b border-primary/10">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <DialogTitle className="text-xl font-black tracking-tight">{product ? 'Modifier le محصول' : 'Nouveau Produit'}</DialogTitle>
                    ...
                    <div className="p-6 space-y-6 max-h-[70vh] overflow-y-auto custom-scrollbar">
                        <div className="grid gap-4">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                ...
                                <div className="space-y-2">
                                    <Label htmlFor="unite" className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">Unité</Label>
                                    <Select value={formState.unite} onValueChange={(value) => setFormState(s => ({ ...s, unite: value as Product['unite'] }))}>
                                        <SelectTrigger id="unite" className="rounded-xl border-none shadow-sm bg-muted/30"><SelectValue /></SelectTrigger>
                                        <SelectContent>
                                    ...
                                    <Popover open={supplierPopoverOpen} onOpenChange={setSupplierPopoverOpen}>
                                        <PopoverTrigger asChild>
                                            <Button variant="outline" role="combobox" className="w-full justify-between rounded-xl h-10 border-none shadow-sm bg-muted/30">
                                                <span className="truncate text-xs">{formState.supplierName || "Choisir..."}</span>
                    ...
                    <DialogFooter className="p-6 bg-card border-t flex gap-3">
                        ...
                        <Button type="submit" disabled={isLoading} className="rounded-xl h-12 font-bold flex-1 shadow-lg shadow-primary/20">
                            ...
                            {product ? 'Mettre à jour' : 'Créer le محصول'}
                        </Button>
                    </DialogFooter>
                </form>
            </DialogContent>
