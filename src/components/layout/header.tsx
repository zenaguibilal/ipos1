...
import {
  Settings, Package, Users2, History, Archive, Wallet, LayoutDashboard,
...
export function AppHeader() {
  ...
  return (
    <header className="flex h-10 items-center gap-2 bg-white border-b px-3 print-hide sticky top-0 z-40 shadow-sm overflow-hidden">
      <div className="flex-shrink-0 flex items-center gap-2">
         <Link href="/dashboard" className="flex items-center gap-1.5 transition-transform active:scale-95">
            ...
            <div className="flex flex-col -space-y-1">
                ...
                <span className="text-[6px] font-black text-primary uppercase">Smart</span>
            </div>
         </Link>
      </div>
          ...
          {allNavLinks.map(link => {
              ...
              return (
                  <Button 
                      ...
                      size="sm"
                      className={cn(
                          "rounded-md font-bold px-2 h-7 text-[9px] uppercase tracking-tight",
      ...
      <div className="flex-shrink-0 flex items-center gap-2">
        <TooltipProvider>
            <Tooltip delayDuration={0}>
                <TooltipTrigger asChild>
                    <button onClick={() => performBackgroundSync()} disabled={isSyncing} className="hidden sm:flex items-center gap-1 text-muted-foreground hover:text-primary transition-colors pr-2 border-r h-5">
                        {isSyncing ? (
                            <RefreshCw className="h-2.5 w-2.5 animate-spin text-primary" />
                        ) : lastSync ? (
                            <Cloud className="h-2.5 w-2.5 text-emerald-500" />
                        ) : (
                            <Cloud className="h-2.5 w-2.5 opacity-30" />
                        )}
        ...
        <div className="flex items-center gap-0.5 ml-1">
            <Button asChild variant="ghost" size="icon" className="h-7 w-7 rounded-md hover:bg-primary/5 hover:text-primary">
