...
export function SaleInfoBar() {
    ...
    const customerName = customer ? `${customer.firstName} ${customer.lastName}` : 'Passager';

    return (
        <div className="bg-primary text-primary-foreground print-hide shadow-sm z-20 relative border-t border-white/5">
            <div className="max-w-[1600px] mx-auto px-3 h-7 flex items-center justify-between">
                <div className="flex items-center gap-3">
                    <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-[8px] font-black bg-white/20 px-1 rounded-sm">{itemCount} items</span>
                    </div>
                    <div className="h-3 w-px bg-white/20" />
                    <div className="flex items-center gap-1.5">
                        <User className="h-3 w-3 opacity-70" />
                        <span className="text-[9px] font-bold">{customerName}</span>
                        {customer && customer.outstandingBalance > 0 && (
                            <span className="text-[8px] font-black bg-rose-500/40 px-1 rounded-sm">Debt: {formatCurrency(customer.outstandingBalance)}</span>
                        )}
                    </div>
                </div>
                ...
                <div className="flex items-center gap-3">
                    <div className="flex items-center gap-1.5">
                        <span className="text-[7px] font-black uppercase opacity-70">Total</span>
                        <span className="text-sm font-black tracking-tight">{formatCurrency(total)}</span>
                    </div>
                    <Button 
                        variant="ghost" 
                        size="icon" 
                        className="h-5 w-5 text-white/40 hover:text-white hover:bg-white/10 rounded-sm"
                        onClick={() => clearCart()}
                    >
